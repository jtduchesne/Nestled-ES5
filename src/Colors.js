"use strict";

(function(Nestled, undefined) {
    /*
    Colors properties:
      output  => Canvas object
    */
    function Colors(opts) {
        this.initCanvas();
        
        this.context = this.canvas.getContext('2d');
        this.imageData = this.context.createImageData(this.canvas.width, this.canvas.height);
        
        var buffer = new DataView(this.imageData.data.buffer);
        for (var i=0; i<64; i++) { //Hardcoded to NTSC for now...
            buffer.setUint8(i*4+0, this.NTSC[i*3+0]);
            buffer.setUint8(i*4+1, this.NTSC[i*3+1]);
            buffer.setUint8(i*4+2, this.NTSC[i*3+2]);
            buffer.setUint8(i*4+3, 0xFF);
        }
        this.context.putImageData(this.imageData,0,0);
        
        if (opts && opts['output'])
            this.attachOutput(opts['output']);
        else
            this.detachOutput();
        
        this.getPixel = function(index) { return buffer.getUint32(index*4, true); };
        this.getR = function(index) { return buffer.getUint8(index*4+0); };
        this.getG = function(index) { return buffer.getUint8(index*4+1); };
        this.getB = function(index) { return buffer.getUint8(index*4+2); };
    }

    Colors.prototype = {
        constructor: Colors,
        
        canvas: document.createElement('canvas'),
        initCanvas: function() {
            this.canvas.width = 16;
            this.canvas.height = 4;
        },
        
        //===============================================================//
        
        attachOutput: function(output) {
            this.output = output;
            this.outputContext = this.output.getContext('2d');
            
            this.outputContext.imageSmoothingEnabled = false;
            
            this.updateOutput();
        },
        detachOutput: function() {
        },
        updateOutput: function() {
            if (this.outputContext)
                this.outputContext.drawImage(this.canvas, 0, 0, 14, 4, 0, 0, this.output.width, this.output.height);
        },
        
        //===============================================================//
        
        NTSC: new Uint8Array([])
    }

    Nestled.Colors = Colors;
})(window.Nestled = window.Nestled || {});